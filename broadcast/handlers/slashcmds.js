...
client.on('ready', async () => {
  ...
  const dirs = fs.readdirSync('/home/runner/Cherno/broadcast/slashCommands/');
  for (const dir of dirs) {
    const files = fs.readdirSync(`/home/runner/Cherno/broadcast/slashCommands/${dir}/`).filter(file => file.endsWith('.js'));

    // Load each slash command and push it to the slashCommands array
    for (const file of files) {
      const slashCommand = require(`/home/runner/Cherno/broadcast/slashCommands/${dir}/${file}`);
      slashCommands.push({
